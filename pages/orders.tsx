...
const Orders: React.FC<OrdersProps> = ({ onNavigate }) => {
  ...
  const getOrderCountsByTab = (orders: Order[]) => {
    return Object.entries(TAB_STATUS_MAP).reduce(
      (acc, [tab, statuses]) => ({
        ...acc,
    ...
    <div className="min-h-screen bg-white">
      <div className="fixed top-0 left-0 right-0 bg-white border-b z-10">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <h1 className="text-xl font-bold">{t("orders.orders")}</h1>
        </div>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              filteredOrders.map((order) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {t("orders.order")} #{order.id} • {order.created_at}
                      </p>
                      <p
                  ...
                  <div className="flex justify-between items-center text-sm text-gray-600 mb-4">
                    <span>
                      Total: {getTotalItems(order.items)}{" "}
                      {t("orders.itemSmall")}
                    </span>
                  ...
                  <div className="border-t pt-4">
                    <div className="grid grid-cols-2 gap-6 text-sm">
                      ...
                      <div>
                        <p className="text-gray-600">{t("orders.orderDate")}</p>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">{t("orders.noOrderFound")}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t">
        <div className="max-w-7xl mx-auto px-6 flex justify-around py-3">
          ...
          >
            ...
            <span className="text-xs mt-1">{t("orders.home")}</span>
          </button>
          <button className="flex flex-col items-center text-blue-600">
            <Package className="h-5 w-5" />
            <span className="text-xs mt-1">{t("orders.orders")}</span>
          </button>
          <button
          ...
          >
            <Package className="h-5 w-5" />
            <span className="text-xs mt-1">{t("orders.settings")}</span>
          </button>
        </div>
      </div>
    </div>
